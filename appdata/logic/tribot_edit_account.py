# \appdata\logic\tribot_edit_account.py
...
def get_tasks_tribot_path():
    try:
        username = os.getlogin()
    except:
        username = os.environ.get("USERNAME", "default")
    return f"C:\\Users\\{username}\\Jivaro\\BotForge\\Data\\tasks_tribot.json"

def load_tasks_tribot():
    path = get_tasks_tribot_path()
    if not os.path.exists(path):
        return []
    try:
        with open(path, "r", encoding="utf-8") as f:
...
class TRiBotEditAccountLogic:
    """
    This class populates combos on init and handles saving/canceling logic for tribot_edit_account
    """
    ...
    def handle_save(self):
        ...
        found["tribot_task"] = self.gui.task_combo.currentText().strip()
        found["world"] = self.gui.world_combo.currentText().strip()
        found["notes"] = self.gui.notes_edit.text().strip()  # replaced setPlainText usage
